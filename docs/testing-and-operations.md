...
| [`pulley.test.ts`](../web/tests/pulley.test.ts) | Equal-tension Atwood acceleration, tension-only slack behavior, live finite-wheel tangency, overlap and swept zero-restitution particle stops without energy spikes, all four Performance profiles, realised contact-force diagnostics, wall mount alignment/following, live snap/breakaway wheel dragging, paused taut axle edits, pulley-only box filtering, complete placement, three dismantling cascades, persistence/digest structure, and malformed axle ownership/reference rejection. |
...
| [`interaction-behaviour.test.ts`](../web/tests/interaction-behaviour.test.ts) | Click activation threshold, asymptotically bounded speed-sensitive drag energy, continuously maintained parked velocity aims, filtered box select, pick ordering/tolerance, distance-based trail sampling, plot restart/non-finite rejection, auto-fit guarantees, and deleted gesture targets. |
...
| [`trail-render.test.ts`](../web/tests/trail-render.test.ts) | Narrow non-`Path2D` full/coarse world-grid, spatial-debug and disjoint-body strokes, square-root-free link slack classification, four-arrow pulley tension geometry and hover column values, dense-lattice coil caps, Performance-mode trail omission, visible/off-screen Normal-mode trail drawing, bounded dense paths, budgets, fading, stable decimation, endpoint/corner retention, and curve fidelity. |
