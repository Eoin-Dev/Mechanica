...
| [`app-lifecycle.test.ts`](../web/tests/app-lifecycle.test.ts) | Exact pre-edit transactions after simulation, undoable replacement routes and atomic failures, history byte limits, first-failure physics batching, strict time jumps, phase/distance/velocity/trail/event rewind cleanup, event-time refinement, playback/reset and configurable Clear gravity, persisted-settings guards, Performance-mode trail suppression/restoration, energy baselines, live-state energy-cache invalidation, graph sampling, and paused-loop energy behavior. |

### Educational analysis

| Test file | Protected behavior |
| --- | --- |
| [`education-analysis.test.ts`](../web/tests/education-analysis.test.ts) | Named free-body force ledgers that close to realised resultants, slope bases/components, ideal pulley tension/rate/acceleration/axle diagnostics, bounded transition detection, event interpolation, and rewind-safe event history. |
...
| [`interaction-behaviour.test.ts`](../web/tests/interaction-behaviour.test.ts) | Click activation threshold, asymptotically bounded speed-sensitive drag energy, continuously maintained parked velocity aims, selected-body FBD velocity-handle suppression independent from View vectors, standalone rod creation, unified Anchor support placement, slow-acquire/hysteretic body attachment, filtered box select, pick ordering/tolerance, distance-based trail sampling, plot restart/non-finite rejection, auto-fit guarantees, and deleted gesture targets. |
