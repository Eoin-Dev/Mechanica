...
| [`string-one-sided.test.ts`](../web/tests/string-one-sided.test.ts) | Elastic strings never push, including when axial damping outweighs small extension, and independent links in Normal/Performance chains publish finite signed axial reactions for analysis. |
...
| [`contact-cache.test.ts`](../web/tests/contact-cache.test.ts) | Warm-start impulse identity across pair ordering changes, explicit wall identity including zero in maximum approximation, and position-only separation at a zero-length capsule centre. |
| [`anchor.test.ts`](../web/tests/anchor.test.ts) | Anchor locking, gravity exclusion, naming/counting, link use, and serialization. |
| [`rod-attachments.test.ts`](../web/tests/rod-attachments.test.ts) | Standalone-beam support/load coupling, exact affine attachment geometry, free and fixed support motion, Normal/maximum Performance long-run energy/speed bounds, impact transfer without deformation energy, persistence/origin identity, deletion release, hidden endpoint cleanup, and coincident-attachment collision exclusion. |
...
| Test file | Protected behavior |
| --- | --- |
| [`scene-guards.test.ts`](../web/tests/scene-guards.test.ts) | Numeric/boolean/colour/name/ID guards and limits, collection-resource caps, duplicate object-ID repair within each identity namespace, anchor/pulley normalization, hidden rod endpoint and orphan-link pruning, live bilateral attachment ownership, settings and link ranges, malformed container shapes, field/driver defaults, missing/self link rejection, and stepping after damaged input. |
...
| [`app-lifecycle.test.ts`](../web/tests/app-lifecycle.test.ts) | Exact pre-edit transactions after simulation, undoable replacement routes and atomic failures, history byte limits, first-failure physics batching, strict time jumps, phase/distance/velocity/trail/event rewind cleanup, selected-body event refinement and newly armed sampling, per-step travel without display-cadence or paused-edit artefacts, playback/reset and configurable Clear gravity, persisted-settings guards, Performance-mode trail suppression/restoration, energy baselines, live-state energy-cache invalidation, graph sampling, and paused-loop energy behavior. |
...
| [`timeseries-render.test.ts`](../web/tests/timeseries-render.test.ts) | Visible-window selection, scrolling/zoom ranges, measured wrapping legends with matching hit regions, axis labels, single points, hidden channels, autoscale, history retention, redraw state, and easing termination for non-plottable data. |
...
| [`panel-accessibility.test.ts`](../web/tests/panel-accessibility.test.ts) | Toolbar/graph icon names, play state labels, unchanged-clock write and seek suppression, Escape cancellation, detached graph zoom at the retained boundary, separated grammatical status counts with internal trail/subdivision indicators absent, revealed/dynamically bounded graph-splitter metadata, retained-graph palette invalidation, and shortcut badges excluded from name computation. |
