...
describe("latex from the math editor maps to the right source", () => {
  const CASES: Array<[string, string]> = [
    ...
    ["\\tau r", "tau*r"],
    ["2\\pi", "2*pi"],
    // the upright \exponentialE with an exponent IS exp(); a plain italic
    // e^x stays a power, so the two spellings never cross-contaminate
    ["\\exponentialE^{-t}", "exp(-t)"],
    ["\\exponentialE", "e"],
    ["\\exponentialE^2x", "exp(2)*x"],
    ["e^{-t}", "e^-t"],
    ["\\left\\lfloor t\\right\\rfloor", "floor(t)"],
    ["\\lfloor t\\rfloor", "floor(t)"],
    ["\\lceil r\\rceil", "ceil(r)"],
    ["2\\left\\lfloor t\\right\\rfloor", "2*floor(t)"],
    ["x\\cdot y", "x*y"],
    ["x\\times y", "x*y"],
    ["x/y", "x/y"],
    ["x\\div y", "x/y"],
